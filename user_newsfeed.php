...
$photoq = mysql_query("select * from photo where User_Register_ID='{$user_id}'");
...
$likeordislike= mysql_query("select * from photo_like where User_Register_ID='{$user_id}' and Photo_ID='".$_GET["photoid"]."'");
$likeordislikecount = mysql_num_rows($likeordislike);
    
   $photoid  = $_GET['photoid'];
    if($likeordislikecount==0)
    {
    mysql_query("insert into photo_like (Photo_ID,Photographer_Register_ID,likes,User_Register_ID) values('{$photoid}','0','1','{$user_id}')") or die(mysql_error());
    }
    else
    {
    mysql_query("delete from photo_like  where Photo_ID='{$photoid}' and User_Register_ID='{$user_id}'") or die(mysql_error());
...
if(isset($_POST["comment"]))
{
    ...
    mysql_query("insert into photo_comment (Photo_Comment_Date,Photo_Comment_Desc,Photographer_Register_ID,Photo_ID,User_Register_ID) values('{$commentdate}','{$comment}','0','{$pid}','{$user_id}')") or die(mysql_error());
              ...
              <li class="dropdown">
                <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Event Zone <span><img src="images/down-arrow.png" alt="" /></span></a>
                  <ul class="dropdown-menu newsfeed-home">
                      ...
                      <li><a href="user_chatroom.php">Event Results</a></li>
...
//                                  $userqcom = mysql_query("select * from user_register where User_Register_ID ='{$photodata[0]}'") or die(mysql_error());
//                                 $userdatacom = mysql_fetch_row($userqcom);
//
//                                echo" <div class='line-divider'></div>
//                              <div class='post-comment'>
//                                <img src='admin/photographers/$user_grph[User_Picture]' alt='' class='profile-photo-sm' />
//                                <p><a href='timeline.html' class='profile-link'>$userdatacom[1] $userdatacom[2] </a> $commentsdata[2]</p>
//                              </div>
//                              ";
                        }
